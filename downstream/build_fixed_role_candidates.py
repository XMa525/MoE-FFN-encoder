...
# loading
# =========================================================
def load_split_csv(split_csv: str, split: Optional[str] = None) -> pd.DataFrame:
...
def score_patches_with_role_proto(
    ...
    if len(neg_ids) == 0:
        raise ValueError(f"No valid negative role names found. got={negative_role_names}, role_names={role_names}")
...
def select_positive_proposal_pool(
    ...
    # -------------------------------------------------
    # weak base mask: only avoid extremely bad patches
    # -------------------------------------------------
    ...
    # -------------------------------------------------
    # branch 1: tumor-evidence proposals
    # -------------------------------------------------
    tumor_pool_score = (
    ...
    # -------------------------------------------------
    # branch 2: context-support proposals
    # -------------------------------------------------
    support_pool_score = (
...
def main():
    ...
    # positive proposal pool defaults
    ...
    # negative fixed candidate defaults
    ...
    for split in args.build_splits:
        df = load_split_csv(args.split_csv, split=split)
